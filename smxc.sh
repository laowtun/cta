#!/bin/bash
# =============================================================================
# smxc.sh - 代理节点部署脚本（speed.cloudflare.com/meta 版）
# 节点名格式: 国家-AS编号-运营商-城市_tls
# 例如: RO-AS214368-Oleksandr Radchenko-Bucharest_tls
...
choose_core() {
    ...
    echo "===================================="
    echo "  smxc.sh - 代理节点部署"
    echo "  (speed.cloudflare.com/meta 版)"
    echo "===================================="
    echo "选择内核:"
    echo "  1. Xray"
    echo "  2. sing-box"
...
download_singbox() {
    ...
    if ! tar xzf singbox.tar.gz -C "$dst" 2>/tmp/sb_err; then err "sing-box 解压失败"; return 1; fi
...
installtunnel() {
    ...
    if [ $(sed 1,2d "$DIR/argo.log" | awk '{print $2}' | grep -w $name | wc -l) == 0 ]; then
        ...
        if [ ! -f "/root/.cloudflared/$(sed 1,2d "$DIR/argo.log" | awk '{print $1" "$2}' | grep -w $name | awk '{print $1}').json" ]; then
            ...
            "$DIR/cloudflared" --edge-ip-version 4 --protocol http2 tunnel delete $name >"$DIR/argo.log" 2>&1
            "$DIR/cloudflared" --edge-ip-version 4 --protocol http2 tunnel create $name >"$DIR/argo.log" 2>&1
        else
            echo "清理 TUNNEL $name"
            "$DIR/cloudflared" --edge-ip-version 4 --protocol http2 tunnel cleanup $name >"$DIR/argo.log" 2>&1
...
main_menu() {
    while true; do
        ...
        echo "===================================="
        echo "  smxc.sh - 代理节点部署"
        echo "  节点名: speed.cloudflare.com/meta"
        echo ""
        echo "  当前内核: $CORE_NAME"
        echo "  安装目录: $DIR"
        echo "===================================="
