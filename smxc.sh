...
if ! [ -t 1 ] 2>/dev/null || [ "$(tput colors 2>/dev/null || echo 0)" -lt 8 ] 2>/dev/null; then RED=''; GREEN=''; YELLOW=''; NC=''; fi
...
install_deps() {
    if is_alpine; then
        ...
        # glibc 兼容层: sing-box 官方二进制是 glibc 动态链接, Alpine(musl) 缺它直接报 "not found" 秒死
        apk add -f gcompat >/dev/null 2>&1
...
choose_core() {
    while true; do
        ...
        echo "  4. 卸载服务 (清理所有节点后返回)"
        echo "  0. 退出"
        read -p "请选择(默认1): " core
        [ -z "$core" ] && core=1
        case "$core" in
            1) CORE=xray;     CORE_NAME="Xray";     CORE_UNIT="smx-xray" ; break ;;
            2) CORE=singbox;  CORE_NAME="sing-box"; CORE_UNIT="smx-singbox" ; break ;;
            3) CORE=mihomo;   CORE_NAME="mihomo";   CORE_UNIT="smx-mihomo" ; break ;;
            4) uninstall_all no; echo ""; read -p "按回车返回内核选择..." _; continue ;;
            0) echo "退出"; exit 0 ;;
            *) warn "请输入 0-4"; sleep 1 ;;
        esac
    done
    DIR="/opt/${CORE}"
...
download_singbox() {
    ...
    mkdir -p "$dst"
    ver=$(curl -fsSL --max-time 10 "https://api.github.com/repos/SagerNet/sing-box/releases/latest" 2>/dev/null | grep -oE "[0-9]+\.[0-9]+\.[0-9]+" | head -1)
    [ -z "$ver" ] && ver="1.13.21"
    case "$arch" in
...
download_mihomo() {
    local dst="$DIR" arch=$(get_arch) ver url
    mkdir -p "$dst"
    ver=$(curl -fsSL --max-time 10 "https://api.github.com/repos/MetaCubeX/mihomo/releases/latest" 2>/dev/null | grep -oE "[0-9]+\.[0-9]+\.[0-9]+" | head -1)
    [ -z "$ver" ] && ver="1.19.30"
    case "$arch" in
        amd64) url="https://github.com/MetaCubeX/mihomo/releases/download/v${ver}/mihomo-linux-amd64-v${ver}.gz" ;;
        arm64) url="https://github.com/MetaCubeX/mihomo/releases/download/v${ver}/mihomo-linux-arm64-v${ver}.gz" ;;
        armv7) url="https://github.com/MetaCubeX/mihomo/releases/download/v${ver}/mihomo-linux-armv7-v${ver}.gz" ;;
        *) warn "mihomo 不支持架构 $arch"; return 1 ;;
    esac
    echo "  下载 mihomo $ver ($arch)..."
    cd "$dst" && rm -rf mihomo.gz mihomo
    curl -fSL --retry 3 --retry-delay 2 -o mihomo.gz "$url" || { err "mihomo 下载失败"; return 1; }
...
download_cloudflared() {
    ...
    curl -fSL --retry 2 -o "$dst/cloudflared" "$url" || { err "cloudflared 下载失败"; return 1; }
    chmod +x "$dst/cloudflared"
    # Alpine/BusyBox 下 tar/gunzip 解压的文件可能丢失执行权限, 再次确认
    [ -x "$dst/cloudflared" ] || chmod +x "$dst/cloudflared"
    echo "  cloudflared 就绪: $dst/cloudflared (mode=$(stat -c %a "$dst/cloudflared" 2>/dev/null || stat -f %Lp "$dst/cloudflared" 2>/dev/null))"
}
download_all() {
...
SEOF
        ...
        # 立即执行一次(不等重启)
        /etc/local.d/${unit}.start >/dev/null 2>&1 &
    else
...
stop_services() {
    ...
    # 先杀进程再删文件 —— gcompat 下进程名变 ld-linux-x86-64, pkill 匹配不到
    # 用 netstat 按端口找 PID, 不依赖进程名
    for pid in $(netstat -tlnp 2>/dev/null | grep -v ":22 " | grep -v "^Proto\|^Active" | grep -oE '[0-9]+/' | sed 's|/||' | sort -u); do
        [ -n "$pid" ] && [ "$pid" -gt 1 ] 2>/dev/null && kill -9 "$pid" 2>/dev/null
    done
    sleep 1
...
quicktunnel() {
    ...
    [ -z "$ipver" ] && ipver=4
    # 先卸载干净: 清除老 systemd unit + 残留 cloudflared, 再 install
    echo "[INFO] 清理旧部署..."
    stop_services
    rm -f /lib/systemd/system/smx-xray.service /lib/systemd/system/smx-singbox.service /lib/systemd/system/smx-mihomo.service /lib/systemd/system/smx-cf.service 2>/dev/null
    sleep 1
    echo "[INFO] 获取节点信息..."
    ...
    sleep 2
    # 自检: 内核必须真在监听端口, 否则 Alpine 缺 glibc(gcompat) 时静默失败, 隧道 530
    if ! { netstat -tln 2>/dev/null | grep -q ":$port " || ss -tln 2>/dev/null | grep -q ":$port "; }; then
        err "$CORE_NAME 未能监听端口 $port(可能二进制无法运行; Alpine 请确认已装 gcompat)"
        exit 1
    fi
...
token_tunnel() {
    ...
    read -p "请选择 argo 连接 IPv4 或 IPv6(输入 4 或 6,默认 4): " ipver
    [ -z "$ipver" ] && ipver=4
    # 部署前清理旧 systemd 单元和残留 cloudflared
    echo "[INFO] 清理旧部署..."
    stop_services
    while true; do
    ...
    # 立即启动内核进程(不等重启); systemd/OpenRC 会自动拉起,但首次需要手动启动
    case "$CORE" in
        mihomo) setsid nohup "$CORE_BIN" -d "$DIR" -f "$DIR/config.yaml" >/dev/null 2>&1 & ;;
        *)      setsid nohup "$CORE_BIN" run -c "$DIR/config.json" >/dev/null 2>&1 & ;;
    esac
    sleep 2
    if is_alpine; then
...
# ---------- 卸载 ----------
uninstall_all() {
    # 模式: 仅"卸载"(不继续跑后续流程)=删除脚本自身; 部署流程复用=保留脚本
    local del_self="${1:-no}"
    for u in smx-xray smx-singbox smx-mihomo smx-cf; do
        systemctl stop ${u}.service >/dev/null 2>&1
        systemctl disable ${u}.service >/dev/null 2>&1
    ...
    # 仅在独立卸载场景(用户显式选卸载)时删除脚本本体; choose_core 里调用时保留脚本继续运行
    if [ "$del_self" = "yes" ]; then
        rm -f /root/smxc.sh /root/smxc1.sh ./smxc.sh 2>/dev/null
        clear; echo "所有服务与脚本都卸载完成"
        exit 0
    fi
    systemctl --system daemon-reload 2>/dev/null
...
clear_cache() {
    ...
    pkill -9 -f "mihomo" 2>/dev/null
    pkill -9 -f "cloudflared" 2>/dev/null
    pkill -9 -f "argv0.*xray" 2>/dev/null
    pkill -9 -f "argv0.*sing-box" 2>/dev/null
    pkill -9 -f "argv0.*mihomo" 2>/dev/null
    for pid in $(netstat -tlnp 2>/dev/null | grep -v ":22 " | grep -v "^Proto\|^Active" | grep -oE '[0-9]+/' | sed 's|/||' | sort -u); do
        [ -n "$pid" ] && [ "$pid" -gt 1 ] 2>/dev/null && kill -9 "$pid" 2>/dev/null
    done
    pkill -9 -f "xray run" 2>/dev/null
