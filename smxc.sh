...
download_singbox() {
    ...
    mkdir -p "$dst"
    ver=$(curl -fsSL --max-time 10 "https://api.github.com/repos/SagerNet/sing-box/releases/latest" 2>/dev/null | sed -n 's/.*"tag_name"[[:space:]]*:[[:space:]]*"v\([0-9.]*\).*/\1/p' | head -1)
    [ -z "$ver" ] && ver="1.13.21"
    case "$arch" in
...
download_mihomo() {
    local dst="$DIR" arch=$(get_arch) ver url
    mkdir -p "$dst"
    ver=$(curl -fsSL --max-time 10 "https://api.github.com/repos/MetaCubeX/mihomo/releases/latest" 2>/dev/null | sed -n 's/.*"tag_name"[[:space:]]*:[[:space:]]*"v\([0-9.]*\).*/\1/p' | head -1)
    [ -z "$ver" ] && ver="1.19.30"
    case "$arch" in
        amd64) url="https://github.com/MetaCubeX/mihomo/releases/download/v${ver}/mihomo-linux-amd64-v1.${ver}.gz"; url2="https://github.com/MetaCubeX/mihomo/releases/download/v${ver}/mihomo-linux-amd64-v1-v${ver}.gz" ;;
        arm64) url="https://github.com/MetaCubeX/mihomo/releases/download/v${ver}/mihomo-linux-arm64-v1-v${ver}.gz"; url2="$url" ;;
        armv7) url="https://github.com/MetaCubeX/mihomo/releases/download/v${ver}/mihomo-linux-armv7-v1-v${ver}.gz"; url2="$url" ;;
        *) warn "mihomo 不支持架构 $arch"; return 1 ;;
    esac
    echo "  下载 mihomo $ver ($arch)..."
    cd "$dst" && rm -rf mihomo.gz mihomo
    curl -fSL --retry 3 --retry-delay 2 -o mihomo.gz "$url" || {
        echo "  主URL失败,备用URL重试中..."
        curl -fSL --retry 3 --retry-delay 2 -o mihomo.gz "${url2:-$url}" || { err "mihomo 下载失败(已尝试备用URL)"; return 1; }
    }
