#!/bin/bash
# ---------------------------------------------------------------
# cs.sh - Cloudflare sing-box 综合部署脚本
#
# 模式:
...
install_deps() {
    ...
    [ -z "$(type -P tar)" ] && { ${linux_update[$n]} >/dev/null 2>&1; ${linux_install[$n]} tar >/dev/null 2>&1; }
...
# =============== 按架构下载 sing-box + cloudflared ===============
# 获取 sing-box 最新版本号
get_singbox_version() {
    local v
    v=$(curl -fsSL --retry 2 "https://api.github.com/repos/SagerNet/sing-box/releases/latest" 2>/dev/null | grep -oP '"tag_name":\s*"\K[^"]+')
    echo "${v#v}"
}
...
download_binaries() {
    ...
    mkdir -p "$dest"
    cd "$dest"
    rm -rf sing-box singbox.tar.gz cloudflared-linux
    echo "[INFO] 正在获取 sing-box 最新版本..."
    local ver=$(get_singbox_version)
    [ -z "$ver" ] && ver="1.13.19"
    echo "[INFO] sing-box 版本: $ver"

    local arch="$(uname -m)"
    echo "[INFO] 正在下载 sing-box + cloudflared (架构: $arch)..."
    case "$arch" in
        x86_64 | x64 | amd64)
            SB_URL="https://github.com/SagerNet/sing-box/releases/latest/download/sing-box-${ver}-linux-amd64.tar.gz"
            CLOUD_URL="https://github.com/cloudflare/cloudflared/releases/latest/download/cloudflared-linux-amd64"
            ;;
        armv8 | arm64 | aarch64)
            echo "[INFO] 检测到 arm64"
            SB_URL="https://github.com/SagerNet/sing-box/releases/latest/download/sing-box-${ver}-linux-arm64.tar.gz"
            CLOUD_URL="https://github.com/cloudflare/cloudflared/releases/latest/download/cloudflared-linux-arm64"
            ;;
        armv7l | armv71)
            SB_URL="https://github.com/SagerNet/sing-box/releases/latest/download/sing-box-${ver}-linux-armv7.tar.gz"
    ...
    # 下载 sing-box
    if ! curl -fsSL --retry 2 --retry-delay 2 -o singbox.tar.gz "$SB_URL"; then
        echo "[ERROR] sing-box 下载失败: $SB_URL"
        echo "请检查网络后重新运行"
        exit 1
    fi
    # 下载 cloudflared
    ...
    # 解压 sing-box
    if ! tar xzf singbox.tar.gz; then
        echo "[ERROR] sing-box 解压失败"
        exit 1
    fi
    chmod +x cloudflared-linux $(find . -name sing-box -type f | head -1)
    rm -rf singbox.tar.gz
    echo "[OK] sing-box 与 cloudflared 下载完成 (位于 $dest)"
}
...
get_singbox_bin() {
    local sb
    sb=$(find "$DIR" -name sing-box -type f 2>/dev/null | head -1)
    echo "$sb"
}
...
quicktunnel() {
    ...
    kill -9 $(ps -ef | grep -E "sing-box|cloudflared" | grep -v grep | awk '{print $2}') >/dev/null 2>&1
    ...
    gen_config "$protocol" "$port" "$uuid" "$urlpath" "$DIR/config.json"

    # 校验配置
    local sb=$(get_singbox_bin)
    validate_config "$sb" "$DIR/config.json" || { echo "[ERROR] sing-box 配置校验失败"; exit 1; }

    "$sb" run -c "$DIR/config.json" >/dev/null 2>&1 &
...
TimeoutStartSec=0
Type=simple
ExecStart=$sb run -c $DIR/config.json
Restart=on-failure
RestartSec=5s
[Install]
...
EOF
    ...
    show_links "$DIR/v2ray.txt"
    echo -e "\n服务已安装, 开机自启已启用. 查看主服务: systemctl status sing-box cloudflared"
}
...
installtunnel() {
    ...
    # 清理旧的安装服务
    if is_alpine; then
        kill -9 $(ps -ef | grep -E "sing-box|cloudflared" | grep -v grep | awk '{print $1}') >/dev/null 2>&1
        rm -rf $DIR /etc/local.d/cloudflared.start /etc/local.d/singbox.start $CMD
    else
        systemctl stop cloudflared.service >/dev/null 2>&1
        systemctl stop sing-box.service >/dev/null 2>&1
        ...
        kill -9 $(ps -ef | grep -E "sing-box|cloudflared" | grep -v grep | awk '{print $2}') >/dev/null 2>&1
        rm -rf $DIR /lib/systemd/system/cloudflared.service /lib/systemd/system/sing-box.service $CMD
        systemctl --system daemon-reload
    fi
...
$sb run -c $DIR/config.json &
EOF
        chmod +x /etc/local.d/cloudflared.start /etc/local.d/singbox.start
        rc-update add local >/dev/null 2>&1
        /etc/local.d/cloudflared.start >/dev/null 2>&1
        /etc/local.d/singbox.start >/dev/null 2>&1
    else
...
while true; do
    ...
    echo sing-box $(systemctl status sing-box.service 2>/dev/null | sed -n '3p')
    echo "1.管理TUNNEL"
    echo "2.启动服务"
    echo "3.停止服务"
    echo "4.重启服务"
    ...
    if [ $menu == 1 ]; then
        ...
        while true; do
            ...
            /opt/cs/cloudflared-linux tunnel list
            echo "1.删除TUNNEL"
            echo "0.退出"
            ...
            if [ $tunneladmin == 1 ]; then
                ...
                /opt/cs/cloudflared-linux tunnel cleanup $tunnelname
                echo "删除TUNNEL $tunnelname"
                /opt/cs/cloudflared-linux tunnel delete $tunnelname
            else
                break
            fi
        done
        ...
        systemctl start sing-box.service 2>/dev/null
        clear
    elif [ $menu == 3 ]; then
        ...
        kill -9 $(ps -ef | grep -E 'sing-box|cloudflared' | grep -v grep | awk '{print $2}') >/dev/null 2>&1
        rm -rf /opt/cs /lib/systemd/system/cloudflared.service /lib/systemd/system/sing-box.service /usr/bin/cs
        systemctl --system daemon-reload
        echo "所有服务都卸载完成"
        exit
    elif [ $menu == 6 ]; then
        clear
        cat /opt/cs/v2ray.txt
    elif [ $menu == 0 ]; then
        echo "退出成功"
        exit
    fi
...
clear_cache() {
    ...
    cd /
    rm -rf $DIR/*.log $DIR/v2ray.txt /tmp/singbox-helper 2>/dev/null
    echo "缓存已清理"
}
...
clear
echo "===================================="
echo "  CS - Cloudflare sing-box 一键脚本"
echo "  Cloudflare sing-box 综合部署 + Radar 节点名"
echo "===================================="
